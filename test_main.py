...
def test_prediction_with_valid_file_shows_prediction(client):
	...
	imgData = {
			"file": (bytes.getvalue(), "test.png")
	}

	response = client.post("/prediction", content_type="multipart/form-data", data=imgData)
	assert b"Prediction" in response.data

def test_prediction_with_invalid_file_shows_error_messages(client):
	txtFile = io.BytesIO(b"")
	txtData = {
			"file": (txtFile, "blank.txt")
	}

	response = client.post("/prediction", content_type="multipart/form-data", data=txtData)
	assert response.status_code == 200
